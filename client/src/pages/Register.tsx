...
export default function Register() {
  ...
  const handleStep1Submit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await api.post("/otp/send", { username, email });
      setStep(2);
    } catch (err: any) {
  ...
  const handleStep3Submit = async () => {
    ...
    setError("");
    try {
      // For GET/POST requests with query params, Axios handles it cleanly
      const optionsRes = await api.post(
        `/passkeys/register/options?email=${encodeURIComponent(email)}`,
      );
      ...
      try {
        ...
        throw new Error("Passkey setup cancelled or failed on device.");
      }
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  <div className="text-center mb-1">
                    ...
                    <p className="text-sm font-medium text-slate-500">
                      Join the competitive ladder today.
                    </p>
                  </div>
                  {error && (
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700 uppercase tracking-wider ml-1 text-center block">
                      6-Digit Code
                    </label>
                    <input
                      type="text"
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-500 px-2">
                      We require passkeys for all accounts to guarantee
                      competitive integrity and prevent account theft.
                    </p>
                  </div>
                  {error && (
                    <p className="text-xs text-red-500 text-center font-semibold bg-red-50 p-2 rounded">
                  ...
                  <div className="flex flex-col mt-4">
                    <button
                      ...
                      disabled={isLoading}
                      className="w-full py-3.5 bg-red-600 hover:bg-red-700 disabled:bg-red-400 text-white font-semibold rounded-xl transition-all shadow-[0_2px_10px_rgb(220,38,38,0.2)] hover:shadow-[0_4px_15px_rgb(220,38,38,0.3)] active:scale-[0.98] flex items-center justify-center gap-2"
                    >
